...
import certifi
import urllib3
from beowulfbase.exceptions import RPCError, RPCErrorRecoverable
...
class HttpClient(object):
    """ Simple Beowulf JSON-HTTP-RPC API

    This class serves as an abstraction layer for easy use of the Beowulf API.

    Args:
      nodes (list): A list of Beowulf HTTP RPC nodes to connect to.

    .. code-block:: python

       from beowulf.http_client import HttpClient

       rpc = HttpClient(['https://beowulfd-node1.com',
       'https://beowulfd-node2.com'])
    ...
    # non_appbase_nodes = set(['http://192.168.1.113:9876/rpc'])
    ...
    def json_rpc_body(name, *args, **kwargs):
        """ Build request body for beowulfd RPC requests.
        ...
        api = kwargs.pop('api', None)  # type: Optional[Any]
    ...
    def call(self,
        ...
        """ Call a remote procedure in beowulfd.
        ...
        while True:
            ...
            #       define exceptions for which we refuse to retry.
                ...
                logger.error('Unexpected exception! Please report at ' +
                             'https://github.com/beowulfchain/beowulf-python/issues' +
