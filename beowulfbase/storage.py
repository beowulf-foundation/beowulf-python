...
from beowulf.aes import AESCipher
...
class DataDir(object):
    ...
    appname = "BWF"
    appauthor = "BeowulfTeam"
    storageDatabase = "beowulf.sqlite"
...
class KeyEncryptionKey(object):
    ...
    def decryptEncryptedKeyFromFile(self):
        """ Decrypt the encrypted KeyEncryptionKey
        """
        aes = AESCipher(self.userPassphrase)
        checksum, encrypted_kek = configStorage[self.config_key].split("$")
    ...
    def getEncryptedKeysToFile(self):
        """ Obtain the encrypted KeyEncryptionKey
        """
        if not self.decrypted_KEK:
            raise Exception("KeyEncryptionKey not decrypted")
        aes = AESCipher(self.userPassphrase)
        cipher_data={}
        cipher_data['cipher_keys'] = aes.encrypt(self.decrypted_KEK)
        cipher_data['checksum_key'] = self.deriveChecksum(self.decrypted_KEK)
        return cipher_data
